...
class BaseTrainer(AbstractTrainer):
    ...
    def fit(self,
        ...
        # ✅ 先判断 tb_logs 是不是一个文件（这是错误的）
        if os.path.isfile("tb_logs"):
            os.remove("tb_logs")  # ⚠ 删除掉错误的 tb_logs 文件

        # ✅ 如果 tb_logs 不是目录，就新建目录
        ...
        if os.path.exists(tb_logs_path):
            shutil.rmtree(tb_logs_path)
        ...
        for epoch in range(num_epochs):
            ...
            if validate_loader is not None:
                ...
                self._show_validation_result(validation_result,
                                             ...
                                             save_best)
                ## 测试用
                #self.logger.info(f"[Epoch {epoch}] Validation metrics: {dict2str(validation_result)}")
